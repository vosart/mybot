...
def constellation(update, context):
    ...
    if not context.args:
        update.message.reply_text('Вы не ввели планету! Чтобы узнать доступные тела, введите /planet ?')
        return
...
def next_full_moon(update, _):
    ...
    update.message.reply_text(f'{emojize(":full_moon:")} Следующее полнолуние {nfm_date}')
